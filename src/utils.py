...
def parse(source_dir):
    filenames = []
    focal_lengths = []
    with open(os.path.join(source_dir, 'image_list.txt').replace('\\','/')) as f:

        for line in f:
            if (line[0] == '#'):
                continue

            (filename, focal_length, *rest) = line.split()

            filenames += [filename]
            focal_lengths += [float(focal_length)]

        img_list = [cv2.imread(os.path.join(source_dir, filename), 1) for filename in filenames]

    return (img_list, focal_lengths)
...
def matched_pairs_plot(p1, p2, mp):
    ...
    for i in range(len(mp)):
        plt.scatter(x=mp[i][0][1], y=mp[i][0][0], c='r',s=3)
